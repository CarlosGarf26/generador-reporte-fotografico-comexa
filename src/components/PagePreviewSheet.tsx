  ...
  if (isVideoReport) {
    ...
    if (isCoverPage) {
      ...
      return (
        ...
        >
          {/* Cover Header Banner: CSIS Regional Command Center Artwork */}
          <div className="rounded-xl w-full h-[115mm] relative overflow-hidden group select-none border border-slate-200 shadow-xs">
            <CsisCoverBanner customUrl={metadata.coverImageUrl} />
            
            {/* Overlay button to replace cover image if user wants */}
            <div className="no-print absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-all z-30">
              <label className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900/80 hover:bg-slate-900 text-white text-xs font-semibold rounded-lg cursor-pointer backdrop-blur-xs shadow-md">
                <ImageIcon className="w-3.5 h-3.5 text-sky-400" />
                <span>{metadata.coverImageUrl ? "Cambiar Imagen" : "Subir Imagen Personalizada"}</span>
                <input
                  type="file"
                  accept="image/*"
                  onChange={(e) => {
          ...
          {/* Cyan to Navy Blue Metadata Block (Matching attached screenshot) */}
          <div className="bg-gradient-to-r from-[#00A3E0] via-[#0072CE] to-[#003B70] rounded-xl p-8 flex-1 my-4 flex flex-col justify-center text-white shadow-md relative overflow-hidden">
            <h2 className="text-3xl font-semibold tracking-normal text-white font-sans mb-1 select-none">
              Regional Command Center
            </h2>
            <p className="text-white text-base font-medium select-none mb-6">
              Evidencia de extracciones de vídeo Dvr´s y Nvr´s)
            </p>

            {/* Fields List */}
            <div className="space-y-3.5 text-sm font-sans select-text">
              <div className="flex items-center">
                <span className="w-48 font-semibold text-white/90 shrink-0 text-left">
                  Sucursal:
                </span>
              ...
              <div className="flex items-center">
                ...
                <span className="font-bold text-base text-white uppercase tracking-wide">
                  {metadata.incidenteTask || "SCTASK0000883852 / REQ0000894862"}
                </span>
              </div>
